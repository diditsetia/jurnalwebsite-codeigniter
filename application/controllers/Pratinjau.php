	...
	defined('BASEPATH') OR exit('No direct script access allowed');
	  ...
	  class Pratinjau extends CI_Controller
	  {
			...
			public function juduljurnal1($id)
		{
			...
			$this->template->keempat('pratinjau1kolom/V_juduljurnal',$data);
		}

			public function pendahuluan1($id)
		{	
			$data['datajurnal'] = $this->webmodel->tampiljurnal($id);
			$this->template->keempat('pratinjau1kolom/V_pendahuluan',$data);	
		}	

			public function kajianpustaka1($id)
		{
			$data['datajurnal'] = $this->webmodel->tampiljurnal($id);
			$this->template->keempat('pratinjau1kolom/V_kajianpustaka',$data);
		}

			public function landasanteori1($id)
		{
			$data['datajurnal'] = $this->webmodel->tampiljurnal($id);
			$this->template->keempat('pratinjau1kolom/V_landasanteori',$data);
		}
			public function metodepenelitian1($id)
		{
			$data['datajurnal'] = $this->webmodel->tampiljurnal($id);
			$this->template->keempat('pratinjau1kolom/V_metodepenelitian',$data);
		}

			public function pembahasan1($id)
		{
			$data['datajurnal'] = $this->webmodel->tampiljurnal($id);
			$this->template->keempat('pratinjau1kolom/V_pembahasan',$data);
		}

			public function penutup1($id)
		{
			$data['datajurnal'] = $this->webmodel->tampiljurnal($id);
			$this->template->keempat('pratinjau1kolom/V_penutup',$data);
		}

			public function daftarpustaka1($id)
		{
			$data['datajurnal'] = $this->webmodel->tampiljurnal($id);
			$this->template->keempat('pratinjau1kolom/V_daftarpustaka',$data);
		}
